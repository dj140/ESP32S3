...
#define LCD_VSYNC_FRONT_NUM         (1)
#define LCD_VSYNC_BACK_NUM          (1)
...
#define ST77903_INS_DATA            (0x32)
#define ST77903_INS_READ            (0x03)
#define ST77903_INS_CMD             (0x02)
#define ST77903_CMD_HSYNC           (0x3C)
#define ST77903_CMD_VSYNC           (0x2C)
...
typedef struct {
    ...
    spi_multi_transaction_t vsync_back_pool[LCD_VSYNC_BACK_NUM];
...
static esp_err_t st77903_qspi_alloc_buffers(st77903_qspi_panel_t *panel)
{
    ...
    ESP_LOGI(TAG, "Frame buffer size: %zu, total: %zuKB", panel->bytes_per_fb, panel->fb_num * panel->bytes_per_fb / 1024);
    ...
    if (panel->bb_size) {
        ...
        ESP_LOGI(TAG, "Bounce buffer size: %zu, total: %zuKB", panel->bb_size, panel->trans_pool_num * panel->bb_size / 1024);
    ...
    ESP_LOGI(TAG, "Trans pool size: %zu, total: %zuKB", panel->trans_pool_size, panel->trans_pool_num * trans_pool_bytes / 1024);
    ESP_LOGI(TAG, "free PSRAM: %d\r\n", heap_caps_get_free_size(MALLOC_CAP_SPIRAM));
    ESP_LOGI(TAG, "free MALLOC_CAP_DMA: %d\r\n", heap_caps_get_free_size(MALLOC_CAP_DMA));
    ...
    ESP_LOGI(TAG, "segment_gap_clock_len: %ld", trans_pool_temp.sct_gap_len);
...
static esp_err_t panel_st77903_qspi_draw_bitmap(esp_lcd_panel_t *handle, int x_start, int y_start, int x_end, int y_end, const void *color_data)
{
    ...
    lcd_write_cmd(panel, 0x2C, NULL, 0);
    // Send the whole frame data
    lcd_write_color(panel);
...
static esp_err_t lcd_write_vsync(st77903_qspi_panel_t *panel, bool is_front)
{
    ...
    // } else {
        spi_device_queue_multi_trans(panel->spi_write_dev, panel->vsync_back_pool, LCD_VSYNC_BACK_NUM, portMAX_DELAY);
    // }
